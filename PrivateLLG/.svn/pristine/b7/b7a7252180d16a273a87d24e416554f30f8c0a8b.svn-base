...
public class ConsumeAdapter extends BaseAdapter {
	...
	public View getView(int position, View view, ViewGroup parent) {
		...
		if(type==1){
			MyFormat.setBitmap(context, holder.consume_Img, item.SHOP_IMG, 100, 100);//消费者图片
		}else if(type==2){
			MyFormat.setBitmap(context, holder.consume_Img, item.CUS_IMG, 100, 100);//店铺图片
		}
		holder.dianName.setText(item.SHOP_NAME);
