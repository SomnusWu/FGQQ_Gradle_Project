...
public class OrderClearActivity extends BaseActivity implements
	...
	Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case 1:// 访问成功
				...
				_youhui.setText("我的酷币:"
						+ prepayCartModelMap.get("myco").toString());
				_youhui0.setText("本次消费可用酷币抵扣:"
				...
				if (prepayCartModelMap.get("compAddress") != null) {
					...
					if(prepayCartModelMap.get("compAddress").equals("请输入收货地址")){
						toast("请输入收货地址");
					}
				}
				break;
			case 2:// 更改地址或发票开头或移除无效商品后后刷新结算单
				...
				if (appContext.isNetworkConnected()) {
					getPrepay("fastGetPrepay", UserInformation
							.getAccess_token());
				} else {
			...
			case 3:// 有无效商品时,移除无效商品
				if (appContext.isNetworkConnected()) {
					getPrepay("removeInvalidSpec", UserInformation.getAccess_token());
				} else {
					toast(R.string.t_Connection);
				}
				break;
			case 4:// 跳转到生成订单界面
				save("save", UserInformation.getAccess_token());
				break;
			case 5:// 跳转到支付页面
	...
	private void init() {
		...
		if (appContext.isNetworkConnected()) {
			...
			getPrepay("forceGetPrepay", UserInformation
					.getAccess_token());
			...
			public void onClick(View v) {
				...
				if (appContext.isNetworkConnected()) {
					...
					if (UserInformation.isLogin()) {
						getPrepay("forceGetPrepay", UserInformation.getAccess_token());
					} else {
...
//		params.addHeader("X-Requested-With", "XMLHttpRequest");
									...
									for (int i = 0; i < shopModels.length(); i++) {
										...
										if (specs.length() > 0) {
											...
											for (int j = 0; j < specs.length(); j++) {
												...
												if(object2.getString("status").equals("WUHUO")){
													map1.put("productDsc","该商品暂无货,提交订单后将被移除");	
												}else if(object2.getString("status").equals("BUSONG")){
													map1.put("productDsc","该商品不在配送范围,提交订单后将被移除");
												}else {
	...
	public void myclick(View v) {
		...
		case R.id.rl_address:// 选择收货地址
			...
			intent.putExtra("position", addressposition);
		...
		case R.id.tijiaodingdan:// 提交订单
			...
			if(address.getText().toString().equals("地址:请输入收获地址")){
				toast("请输入收货地址");
				return;
			}
			changeInvoice("containsInvalidSpec", "", "", UserInformation.getAccess_token());
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (data != null ) {
			...
			addressposition=data.getIntExtra("position", -1);
...
//			intent.putExtra("street", adapter.getItem(arg2).getAddress());
	...
	public void setPreferential(String coin, String voucher) {
		...
		if (coin.length() > 0 && voucher.length() > 0) {
			youhui.setText("酷币可抵扣：" + coin + "元," + "满省优惠已省：" + voucher);
		} else if (coin.length() > 0 && voucher.length() < 1) {
			youhui.setText("酷币可抵扣：" + coin + "元");
	...
	public void getInvoiceType(String invoiceHead, String invoiceCategory,
		...
		changeInvoice("changeInvoice", invoiceType, invoiceHead, UserInformation.getAccess_token());
	}
}
