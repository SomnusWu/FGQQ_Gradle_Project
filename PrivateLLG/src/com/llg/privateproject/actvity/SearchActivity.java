...
public class SearchActivity extends BaseActivity implements TextWatcher {
	...
	/** 搜索结果列表 */
	private List<MyHistoryModel> listSearch_result;
	/** 热搜列表适配器 */
	...
	private void init() {
		...
		if (getIntent().getStringExtra("name") != null) {
			Log.d("my", "name" + getIntent().getStringExtra("name"));
			home_search_id.setText(getIntent().getStringExtra("name"));
			Log.d("my", "name" + getIntent().getStringExtra("categoryId"));
		}
		listSearch_result = new ArrayList<MyHistoryModel>();
		...
		historyAdapter = new ArrayAdapter<String>(this,
				...
				listhistory);
		searchResultAdapter = new CommodityAdapter(this, listSearch_result);
	...
	private void testHot() {
		String str = getSharedPreferences("suggestSearch", Context.MODE_PRIVATE)
				.getString("suggestSearch", "");
		try {
	...
	private void search(String keyword) {
		...
		AppContext.getHtmlUitls().xUtils(this, HttpMethod.POST, "search",
					...
					public void onBack(JSONObject json) {
						...
						try {
							if (json.getBoolean("success")) {
								JSONObject searchResult = json.getJSONObject(
										"attributes").getJSONObject(
										"searchResult");
								JSONArray suggestSearch = searchResult
										.getJSONArray("suggestSearch");

								getSharedPreferences("suggestSearch",
										Context.MODE_PRIVATE)
										.edit()
										.putString("suggestSearch",
												suggestSearch.toString())
										.commit();
...
//									}
									...
									listSearch_result.addAll(model.mJsonData(productList.toString()));
	...
	protected void onPause() {
		...
		getSharedPreferences("suggestSearch", Context.MODE_PRIVATE).edit()
				.putString("history", json.toString()).commit();
	}
		...
		case R.id.home_sweep_id:// 搜索按钮
			if (home_search_id.getText().toString().trim().length() < 1) {
				...
				if (appContext.isNetworkConnected()) {
					search(home_search_id.getText().toString().trim());
				} else {
	...
	public void onItemClicks(AdapterView<?> parent, View arg1, int position,
		...
		case R.id.search_result_gv:// 搜索结果列表
			if (appContext.isNetworkConnected()) {
				MyHistoryModel model = listSearch_result.get(position);
				intent.putExtra("prodspecId", model.getProdspecid());
				intent.putExtra("img", model.getIconpath());
//				intent.putExtra("prodspecId", listSearch_result.get(position)
//						.get("specId").toString());
//				intent.putExtra("img",
//						listSearch_result.get(position).get("img").toString());
				startActivity(intent);
			} else {
				toast(R.string.t_Connection);
			}
