...
public class GuanzhuShopAdapter extends BaseAdapter {
	...
	/** 关注窗口店铺列表适配器 */
	public GuanzhuShopAdapter(Context context, List<MyShopModel> list,
			boolean isShowcb) {
	...
	public View getView(int position, View v, ViewGroup parent) {
		...
		if (v == null) {
			...
			//
			// } else {
			// score = (int) (Float.parseFloat(map.get("score").toString()) /
			// 1);
			// }
			// lp1.width = MyFormat.px2dip(context,
			// Float.parseFloat(map.get("score").toString()) * 20);
		...
		final MyShopModel model = list.get(position);

		MyFormat.setBitmap(context, holder.shopHead, model.getICON_PATH(), 0, 0);
		holder.shopName.setText(model.getNAME());
		holder.shopDsc.setText(model.getMAINSELL());
		holder.shopTrends.setText(model.getATTENTION_COUNT());
		// holder.shopScore.setText("评分" + map.get("score").toString());
