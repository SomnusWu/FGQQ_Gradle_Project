 ...
 * 订单状态适配器 yh 2015.8.1
 * */
public class OrderStatusAdapter1 extends BaseAdapter {
	Context context;
	 ...
	 * @param dialog12
	 */
	public OrderStatusAdapter1(Context context, List<Map<String, Object>> list) {
		super();
	...
	public View getView(final int position, View v, ViewGroup parent) {
		...
		if (v == null) {
			...
			listener = new OnClickListener() {
				...
				public void onClick(View v) {
					...
					case R.id.pay:// 立即支付
						...
						intent.putExtra("code", map.get("orderPayCode")
								.toString());
						intent.putExtra("id", map.get("orderPayId")
								.toString());
					...
					case R.id.confirm:// 确认收货
						OrderStatusDialog dialog3 = new OrderStatusDialog(
								context, 11, map.get("id").toString());
						dialog3.show();
					...
					case R.id.pingjia:// 评价
						intent.setClass(context, OrderStatuspingjia.class);
						context.startActivity(intent);
			...
			if (map.get("orderStatus").toString().equals("待发货")) {
			...
			}else if(map.get("orderStatus").toString().equals("待收货")){
				holder.pay.setVisibility(View.GONE);
				holder.cancel.setVisibility(View.GONE);
				holder.contact.setVisibility(View.GONE);
				holder.delete.setVisibility(View.GONE);
				...
				holder.confirm.setTextColor(context.getResources().getColor(R.color.orange2));
				holder.confirm.setBackgroundResource(R.drawable.stroke_grayline_r2);
				holder.pingjia.setVisibility(View.GONE);
				holder.refund.setVisibility(View.GONE);
				holder.return_status.setVisibility(View.GONE);
		...
		if (map.get("orderPayCode") == null) {
			holder.shop_name.setText("付款编号:\n" + map.get("id").toString());
		} else {
			holder.shop_name.setText("订单编号:"
		...
		for (int i = 0; i < listProducts.size(); i++) {
			...
			TextView product_count = (TextView) view
					.findViewById(R.id.product_count);
			MyFormat.setBitmap(context, product_img,
					map2.get("img").toString(), MyFormat.dip2px(context, 50),
					MyFormat.dip2px(context, 50));
			product_dsc.setText(map2.get("prodName").toString());
			product_price.setText(MyFormat.getPriceFormat(map2
					.get("finalPrice").toString().trim()));
			product_price.setVisibility(View.VISIBLE);
			product_count.setVisibility(View.VISIBLE);
			product_count.setText("X" + map2.get("amount").toString());
			view.setTag(i);

			holder.ll_product.addView(view);
		}
		holder.dsc = map.get("businessName").toString() + ",";
		...
		v.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				OrderStatusDialog dialog2 = new OrderStatusDialog(context, 2,map.get("id").toString(),"2");
