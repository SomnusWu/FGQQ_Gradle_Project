...
public class PayActivity extends FragmentActivity implements Mydismiss {
	...
	private double totalBalances;// 需要支付的价格-使用余额部分后剩余的实际支付价格
	...
	public void pay(View v) {
		...
		if (cbisChecked) {
			if (iszhi) {// 支付宝
				/**
				 * 当用户余额大于==支付价格,直接把价格设置为支付价格传过去 小于的时候,把余额保留两位传进去
				 */
				if (totalBalances == 0) {// Double.parseDouble(payPrice)
	...
	public void toweixin(String asset) {
		if (!CommonUtils.isWeixinAvilible(this)) {
			Toast.makeText(PayActivity.this, "未检测到微信客户端,请检查重试!",
					Toast.LENGTH_SHORT).show();
			return;
		}
		RequestParams params = new RequestParams();
